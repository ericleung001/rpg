...
io.on('connection', (socket) => {
    ...
    socket.on('register', (data) => { 
        ...
        //  [修正] 0. 檢查 IP 是否被封鎖 (防止被 Ban 的人註冊分身)
        // 注意：這依賴於前面定義的 getSocketIp 函式和 BanSystem 模組
        const clientIp = getSocketIp(socket);

        //  修正點：使用 isIpBanned 替代舊的 isBanned
        if (typeof BanSystem !== 'undefined' && BanSystem.isIpBanned(clientIp)) {
             socket.emit('authResult', { success: false, msg: "⛔ 此 IP 已被封鎖，無法註冊新帳號" });
             return;
        }

        //  [原有] 1. 安全性檢查 (擋下隱形字元、空格、非法符號)
        ...
        //  [原有] 4. 再次確保 HTML 轉義 (雙重保險)
        const safeUser = escapeHtml(user);

        DB.createAccount(safeUser, hash, newToken, (success, msg) => { 
    ...
    socket.on('login', (data) => { 
        ...
        DB.getUserInfo(user, (info) => {
            if (!info) { socket.emit('authResult', { success: false, msg: "帳號不存在" }); } 
                ...
                //  [新增] 2. 帳號封鎖檢查 (第二道防線)
                // 確認帳號存在後，檢查該帳號是否被 Ban
                if (typeof BanSystem !== 'undefined') {
                    const accBan = BanSystem.isAccountBanned(user);
                    if (accBan) {
        ...
        jwt.verify(token, JWT_SECRET, (err, decoded) => {
            ...
            DB.loadPlayer(token, (savedData) => { 
                if (savedData) { 
                    ...
                    if (p.lotteryLastDate !== todayStr) {
                        p.lotteryCount = 0;
                        p.lotteryLastDate = todayStr;
                    }
...
//  [新增] Admin 查詢玩家 IP
    ...
    socket.on('sendChat', (data) => {
        ...
        if (player && msg && msg.trim().length > 0) {
            ...
            // 如果帶了 roomId 且房間存在，只廣播給房間內玩家，並標記 isRoomChat
            if (roomId && gameState.battleRooms[roomId]) {
                io.to(roomId).emit('chatMessage', { ...chatObj, isRoomChat: true });
            } else {
    ...
    socket.on('marketSell', (data) => {
        ...
        // 檢查數量是否足夠
        if (!p.inventory[itemId] || p.inventory[itemId] < countToSell) { 
            socket.emit('errorMessage', `你沒有足夠的數量 (擁有: ${p.inventory[itemId] || 0})`); 
            return; 
        }
    ...
    socket.on('npcSell', (data) => { 
        ...
        if (currentQty < amount) {
            socket.emit('bagResult', { success: false, msg: " 物品數量不足！" }); 
            return; 
        }
    ...
    socket.on('createRoom', (monsterKey) => { 
        ...
        try { 
            ...
            gameState.battleRooms[roomId] = { 
                ...
                monsters: monsters, //  改存 monsters 陣列
                monster: monsters[0], // 兼容舊代碼 (指向第一隻)
                status: 'waiting', 
                players: [socket.id], 
                host: socket.id, 
                hostName: p.name, 
            ...
            // 5. 初始化回合順序 (重要！)
            // 這裡還在 waiting 狀態，真正戰鬥開始會在 startBattle 再次洗牌
            // 但為了防呆，我們先放進去
    ...
    socket.on('joinRoom', (roomId) => { const p = gameState.players[socket.id]; if (!p) return; if (p.hp <= 0) { socket.emit('errorMessage', " 你已經死亡！請先去旅館復活。"); return; } const room = gameState.battleRooms[roomId]; if (room && room.status === 'waiting') { if (room.players.length >= 5) { socket.emit('errorMessage', '❌ 房間已滿 (上限 5 人)！'); return; } if (!room.players.includes(socket.id)) room.players.push(socket.id); socket.emit('roomJoined', { roomId: roomId, isHost: false }); io.to(roomId).emit('roomInfoUpdate', getRoomPublicInfo(room)); broadcastHubData(); } else { socket.emit('errorMessage', '無法加入'); } });
    ...
    socket.on('handleGuildApply', (data) => {
        ...
        if (action === 'reject') {
            ...
            if (g.members.length >= 50) {
                socket.emit('errorMessage', "公會成員已滿 (50人)");
                return;
            }
    ...
    room.players.forEach(pid => {
        ...
        if (p.hp > 0) {
            ...
            if (typeof gainExp === 'function') {
                gainExp(p, personalTotalExp, pid);
            } else {
                p.exp += personalTotalExp;
                if (typeof checkLevelUp === 'function') checkLevelUp(p);
            }
            ...
            // 通知勝利 (有獎勵)
            io.to(pid).emit('battleWon', { exp: personalTotalExp, gold: totalBaseGold, drops: myDrops });
...
function processNextTurn(room, roomId) {
    ...
    while (!validTargetFound && loopCount < 30) {
        ...
        if (monsterAgent) {
            ...
            // 通知前端更新回合 (顯示怪物名字，鎖定玩家按鈕)
            broadcastTurn(room, monsterAgent.id, monsterAgent.name);
...
function checkLotteryLimit(p, amountToBuy) {
        ...
        // 1. 確保變數存在
        if (typeof p.lotteryCount !== 'number') p.lotteryCount = 0;
...
function calculateStats(p) {
    ...
    Object.keys(p.equipment).forEach(slot => {
        ...
        if (itemId && ITEM_CONFIG[itemId]) {
            ...
            //  [關鍵修改] 這裡改用指數成長 (讓高強化更有價值)
            // +10 = 2.6倍, +20 = 6.7倍, +30 = 17.4倍
            let multiplier = 1;
            if (lv > 0) {
