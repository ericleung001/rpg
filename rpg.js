...
//  [新增] Admin 查詢玩家 IP
    ...
    socket.on('joinRoom', (roomId) => { const p = gameState.players[socket.id]; if (!p) return; if (p.hp <= 0) { socket.emit('errorMessage', " 你已經死亡！請先去旅館復活。"); return; } const room = gameState.battleRooms[roomId]; if (room && room.status === 'waiting') { if (room.players.length >= 5) { socket.emit('errorMessage', '❌ 房間已滿 (上限 5 人)！'); return; } if (!room.players.includes(socket.id)) { room.players.push(socket.id); socket.join(roomId); } socket.emit('roomJoined', { roomId: roomId, isHost: false }); io.to(roomId).emit('roomInfoUpdate', getRoomPublicInfo(room)); broadcastHubData(); } else { socket.emit('errorMessage', '無法加入'); } });
